...
namespace sjtu
{
    ...
    public:
        ...
        auto Put(std::string_view key, T value) const -> Trie
        {
            ...
            {
                par = cur;
                auto new_node = (cur->children_.find(ch) != cur->children_.end()) ? std::shared_ptr<TrieNode>(cur->children_.at(ch)->Clone()) : std::make_shared<TrieNode>();
                cur->children_.at(ch) = new_node;
                cur = new_node;
            }
            if (!key.empty())
            {
                auto ch = key.back();
                par->children_.at(ch) = std::make_shared<TrieNodeWithValue<T>>(std::move(cur->children_), std::move(value_ptr));
            }
            else
        ...
        auto Remove(std::string_view key) const -> Trie
        {
            ...
            for (int i = 0; i < path.size() - 1; i++)
                path[i]->children_.at(key[i]) = path[i + 1];
            ...
            if (!path.back()->children_.empty()) // end_node can't be deleted
            {
                ...
                path[path.size() - 2]->children_.at(key.size() - 1) = new_node;
            }
            else
            {
    ...
    public:
        ...
        void Put(std::string_view key, T value)
        {
            ...
            auto snap_trie = root_.Put<T>(key, std::move(value));
            {
                std::lock_guard<std::mutex> lock(root_lock_);
                root_ = snap_trie;
            }
        ...
        {
            std::lock_guard<std::mutex> lock(write_lock_);
            auto snap_trie = root_.Remove(key);
            {
                std::lock_guard<std::mutex> lock(root_lock_);
                root_ = snap_trie;
            }
