...
void test2()
{
    ...
    // for (int i = 0; i < num; i++)
    // {
    //     std::cerr << i << ": " << q[i].num() << " " << stl[i].num() << std::endl;
    // }
    for (int i = 0; i < 100; i++)
    {
        int t = rand() % num;
        ...
        {
            bool judge1 = (q[t] != stl[t]);
            bool judge2 = (q.at(t) != stl.at(t));
            // std::cerr << q[t].num() << " " << stl[t].num() << std::endl;
            puts("Wrong Answer");
            return;
        }
    }
...
void test5()
{
    ...
    //     std::cerr << q[i].num() << " " << stl[i].num() << std::endl;
    for (int i = 1; i <= sqrt(N) && q.size() >= 10; i++)
    {
        ...
        // std::cerr << i << ": " << (*it_q).num() << " " << (*it_stl).num() << std::endl;
        it_q = q.erase(it_q);
        it_stl = stl.erase(it_stl);
        // std::cerr << i << ": " << (*it_q).num() << " " << (*it_stl).num() << std::endl;
        it_q->change(t);
