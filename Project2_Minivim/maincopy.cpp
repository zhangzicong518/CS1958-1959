...
MiniVim::MiniVim(const std::string &file, std::string ttype = "-t")
{
    ...
    else
    {
        ...
        if (filename.empty())
            filename = "untitled.txt";
        if (ExecCmd((query + address + filename).c_str(), result) != -1)
            isdos = TypeCheck(result);
        // to nake it compatible for dos file
        if (isdos)
            ExecCmd((TransToUnix + address + filename).c_str(), result);
    }
    load_file();
    init();
}
...
MiniVim::~MiniVim()
{
    ...
    if (isdos)
        system((TransToDos + address + filename).c_str());
    refresh();
    noraw();
    echo();
    endwin();
...
{
    while (mode == 'n' && (int)content[now_line].length() >= 1 && now_column >= content[now_line].length())
        left();
    PrintCurse();
}
...
loop:
    ...
    case 8: // search & substitution
        ...
        cursor_column = std::min(cursor_column, (int)content[now_line].length());
        ColumnReset();
        mode = 'n';
        NormalBack();
        mode = 'c';
