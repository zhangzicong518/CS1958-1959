...
namespace sjtu
{
    ...
    public:
        ...
        {
            hashmap new_table;
            new_table.capacity = GetMod(capacity);
            new_table.table = new node *[new_table.capacity + 1];
        ...
        pair<iterator, bool> insert(const value_type &value_pair)
        {
            ...
            node *tmp = new node();
            tmp->val = new value_type(value_pair);
            tmp->nxt = table[index];
            if (table[index] != nullptr)
                table[index]->pre = tmp;
            table[index] = tmp;
            size++;
            res.first = iterator(tmp);
            res.second = true;
    ...
    public:
        ...
        pair<iterator, bool> insert(const value_type &value)
        {
            ...
            if (mit == HashMap.end())
            {
                ...
                pair<Key, iterator> tmp_value;
                tmp_value.first = value.first;
                tmp_value.second = --List.end();
                HashMap.insert(tmp_value);
                ++_size;
                res.first = tmp_value.second;
                res.second = true;
            }
            else
            {
    ...
    public:
        ...
        Matrix<int> *get(const Integer &v) const
        {
            ...
            Matrix<int> *res = new Matrix<int>((*(it)).second);
            return res;
        }
